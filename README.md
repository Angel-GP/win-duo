...
- 抓屏优先走 WGC（Windows.Graphics.Capture），其次 DXGI（`bettercam`），装不上自动
  退回 mss（帧率上限会降低，功能不受影响）。可用 `--capture-backend` 强制指定。
- **待命自动省内存**（默认行为）：玻璃层隐藏满 3 秒后自动把 ~120MB 的显卡驱动
  DLL 页从工作集裁出去，GL 窗口保留——下次合盖**零重建延迟**，内存回落几十 MB。
- 可选**低内存模式**（设置 → 高级 → 调试，或 `--low-memory`）：待命时直接销毁 GL
  窗口，省得更多；代价是合盖时要现建窗口，有约 0.4 秒延迟。默认关。
- 非摄像头路径（串口/键盘角度源、纯托盘待命）**不加载 opencv**（~116MB），opencv
  只在真正启动摄像头源时才进内存。
